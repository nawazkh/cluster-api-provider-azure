...
def deploy_worker_templates(template, substitutions):
    ...
    if "windows" in flavor_name:
        ...
        ''' + kubectl_cmd + ''' --kubeconfig ./${CLUSTER_NAME}.kubeconfig create namespace calico-system --dry-run=client -o yaml | \
        ''' + kubectl_cmd + ''' --kubeconfig ./${CLUSTER_NAME}.kubeconfig apply -f -;
        ''' + kubectl_cmd + ''' --kubeconfig ./${CLUSTER_NAME}.kubeconfig get configmap kubeadm-config --namespace=kube-system -o yaml | \
